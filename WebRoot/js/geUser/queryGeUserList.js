...
layui. addGeUserData = function()
{
    ...
    layer.open({
        ...
        closeBtn : 1,
        area: ['330px', '180px'],
        content: addr + '/view/geUser/addGeUser.html'
    });
...
layui.use('table', function(){
    ...
    table.render({
        elem: '#geUserTable'
        ,url: addr + "/geUser/queryGeUser"
        ,cellMinWidth: 80 //全局定义常规单元格的最小宽度，layui 2.2.1 新增
        ,cols: [[
             {type:'checkbox'} ///如果不要选择,可以去掉此列

            ,  {field: 'id', title: i18next.t('id')}
                ,  {field: 'loginName', title: i18next.t('loginName')}
                ,  {field: 'password', title: i18next.t('password')}
    ...
    table.on('tool(tableFilter)', function(obj){
        ...
        if(obj.event === 'detail'){
            ...
            layer.open({
                ...
                closeBtn : 1,
                area: ['300px', '500px'],
                content: addr + '/view/geUser/editGeUser.html?id=' + data.id
            });
        }
    ...
    function del(id) {
        ...
        $.ajax(
        {
            url: addr + '/geUser/deleteGeUser?id='+id,
            type: "post",
            async: false,
